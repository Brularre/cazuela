...
description: >
  Create an entry in agent_log.txt documenting a recent AI suggestion
  and the decision made about it. Use after any architectural or
  significant implementation decision.
argument-hint: "[topic hint — optional]"
---

Create an entry in `agent_log.txt` documenting a recent AI suggestion
and the decision made about it.

## Steps

1. Run `git diff HEAD` and `git status` to see what changed recently
   (if anything)
2. Read `agent_log.txt` to understand the existing format and avoid
   duplicates
...
- **Line length:** break long lines at natural clause or sentence
  ...
  Aim for ~80 characters max per line.
