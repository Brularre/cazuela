...
def route(message: str, user: dict) -> str:
    ...
    match = REMINDER_SET_PATTERN.match(message)
    if match:
        if guard := _module_guard("set_reminder", user):
            return guard
        captured = match.group(1).strip()
        remind_at, ambiguous = parse_time_meta(captured)
        ...
        return stage_reminder(fragment, remind_at, user, recur=recur, ambiguous=ambiguous)
